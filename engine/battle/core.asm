...
GiveEXPToMonsThatNeedIt:: ; give EXP to mons that fought
	xor a
...
.menuselected
	...
	jp .handleBattleMenuInput ; handle menu input if it's not the old man tutorial
...
.loop
	...
	cp $ff					; if we've reached the end of the table (no matching pair)
	jr z, StoreDamage		; jump out
	cp b					; does move type match "attacking type"?
	jr nz, .nextTypePair	; if not, go to the next pair
	...
	jr z, .matchingPairFound
	jr .nextTypePair			; if not, go to the next pair
...
.bideEffect
	...
	ld [wEnemyMoveEffect], a
	ld a, 2
	ld [bc], a ; set Bide counter to 2 turns (not random)
	ld a, [H_WHOSETURN]
	add XSTATITEM_ANIM
